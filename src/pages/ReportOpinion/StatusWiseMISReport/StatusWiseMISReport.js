...
export default function StatusWiseMISReport(props) {
  ...
  const onSubmit = async (event) => {
    ...
    let sendPost = {
      ...
      receiveDate: fromElementsData.receiveDate.checked,
      sendOnDate: fromElementsData.sendOnDate.checked,
    }

    sendPost = JSON.stringify(sendPost);
    sendPost = encodeURI(sendPost);

    navigate(`/app/reportOpinion/statusWiseMISReport/PDFRenderStatusWiseMISReport/${sendPost}`, { replace: true,  });
  ...
  return (
    <Page title="Status Wise MIS Report">
      <Container maxWidth="xl">
        {/* <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
          <Typography variant="h4" gutterBottom>
          Status Wise MIS Report
          </Typography>
