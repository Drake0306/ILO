...
import { useNavigate, useParams } from 'react-router-dom';import Pdf from "react-to-pdf";
import './pdfStyle.css';
...
export default function PDFRenderBankWiseMISReport (props) {
    ...
    const exportToExcel = async () => {
        ...
        // FileSaver.saveAs(data, `Export All Status Wise Cases Received${fileExtension}`);
        ...
        // Create worksheet
        const worksheet = XLSX.utils.aoa_to_sheet([['All Status Wise Cases Received'], [], headers, ...dataArray]);
        ...
        XLSX.writeFile(workbook, 'Export All Status Wise Cases Received.xlsx');
    }
    ...
    const [dd, setDD] = useState({
        ...
        content: [
            {text: 'All Status Wise Cases Received', style: 'header'},
            {text: `                                                                              From: ${moment(paramsData.from).format('DD-MM-YYYY')}     To: ${moment(paramsData.to).format('DD-MM-YYYY')}                                                 As on Date: ${moment().format('DD-MM-YYYY')}`, style: 'subheader'},
            {
                style: 'tableExample',
                table: {
                    body: [
