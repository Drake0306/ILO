...
export default function RegistrationBank(props) {
  ...
  const onSubmit = async (event) => {
    ...
    if(typeof props.name !== 'undefined') {
      ...
      sendPost.regiLedger = false;
      
      sendPost = JSON.stringify(sendPost);
      sendPost = encodeURI(sendPost);
      
      navigate(`/app/reportDisbursal/loanBank/PDFRenderLoanBank/${sendPost}`, { replace: true,  });
    } else {
      ...
      navigate(`/app/reportDisbursal/registrationBank/PDFRenderRegistrationBank/${sendPost}`, { replace: true,  });
    }
  };
