...
export default function Branch() {
  ...
  const redirectPage = async (url) => {
    if(url === "homepage") {
      ...
      navigate(`/app/master/branch/${url}`, { replace: true });
    }
  }

  useEffect(() => {
    axios.get(`${JSON_CONST.DB_URL}master/branch/list`)
     .then((response) => {
  ...
  return (
    <Page title="Branch">
      ...
      <Container maxWidth="">
        ...
        <Card sx={{ width: '100%' }}>
          <UserListToolbar selected={selected} data={USERLIST} searchName={"branch Name"} numSelected={selected.length} filterName={filterName} onFilterName={handleFilterByName} />
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { id, name, bankName, contactPerson, address, email, phoneOne, status, avatarUrl } = row;
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell align="right">
                          <UserMoreMenu urlTo={'/app/master/branch/newEntry/'} data={row} dataID={id} deleteURL={'master/branch/delete/'} />
                        </TableCell>
                      </TableRow>
                    );
                  })}
