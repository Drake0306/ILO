...
export default function EntryFormD(props) {
  ...
  useEffect(() => {
    ...
    const onSubmit = async (event) => {
      ...
      if(params.data !== '0') {
        try {
          await axios.post(`${JSON_CONST.DB_URL}master/delTable/update/${sendPost.id}`, sendPost)
            .then((response) => {
              console.log(response);
              setIsSubmitting(false);
              navigate(`/app/master/del`, { replace: true });
            })
            .catch((error) => {
              setIsSubmitting(false);
              console.log(error);
          ...
          try {
            await axios.post(`${JSON_CONST.DB_URL}master/delTable/create`, sendPost)
              .then((response) => {
                console.log(response);
                setIsSubmitting(false);
                navigate(`/app/master/del`, { replace: true });
  ...
  return (
    <Page title="Del">
      <Container maxWidth="xl">
        ...
        {/* <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
          <Button variant="outlined" color="info" onClick={() => redirectPage('del')} startIcon={<Iconify icon="akar-icons:arrow-back" />}>
            View List
          </Button>
        </Stack> */}
