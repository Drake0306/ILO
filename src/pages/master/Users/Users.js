...
const TABLE_HEAD = [
  ...
  { id: 'email', label: 'User Name', alignRight: false },
...
export default function Users() {
  ...
  const redirectPage = async (url) => {
    if(url === "homepage") {
      ...
      navigate(`/app/master/user/${url}`, { replace: true });
    }
  }

  useEffect(() => {
    axios.get(`${JSON_CONST.DB_URL}auth/userList`)
     .then((response) => {
  ...
  return (
    <Page title="Users Master">
      ...
      <Container maxWidth="">
        ...
        <Card sx={{ width: '100%' }}>
          <UserListToolbar selected={selected} data={USERLIST} searchName={"fee Name"} numSelected={selected.length} filterName={filterName} onFilterName={handleFilterByName} />
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { id, name, particulars, FeeinRs, address, email, phoneOne, status, avatarUrl } = row;
                      ...
                      <>
                        ...
                        >
                          ...
                          <TableCell component="th" scope="row" padding="none">
                            <Stack direction="row" alignItems="center" spacing={2}>
                              <Typography variant="subtitle2" noWrap >
                              {name === 'admin' ? <p title={row.password}>Hover To View password ******</p> : '******'}
                              </Typography>
                            </Stack>
                          </TableCell>
                          ...
                          {name === 'admin' ? null : 
                            <TableCell align="right">
                              <UserMoreMenu urlTo={'/app/master/user/newEntry/'} data={row} dataID={id} deleteURL={'auth/userDelete/'} />
                            </TableCell>
                          }
