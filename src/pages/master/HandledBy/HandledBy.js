...
export default function HandledBy() {
  ...
  const redirectPage = async (url) => {
    if(url === "homepage") {
      ...
      navigate(`/app/master/handledBy/${url}`, { replace: true });
    }
  }

  useEffect(() => {
    axios.get(`${JSON_CONST.DB_URL}master/handledBy/list`)
     .then((response) => {
  ...
  return (
    <Page title="Handled By">
      <Container maxWidth="">
        ...
        <Card>
          <UserListToolbar selected={selected} data={USERLIST} searchName={"Handled By Names"} numSelected={selected.length} filterName={filterName} onFilterName={handleFilterByName} />
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell align="right">
                          <UserMoreMenu urlTo={'/app/master/handledBy/newEntry/'} data={row} dataID={id} deleteURL={'master/handledBy/delete/'} />
                        </TableCell>
                      </TableRow>
                    );
                  })}
