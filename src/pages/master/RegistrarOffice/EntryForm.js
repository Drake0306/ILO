...
export default function EntryFormRO(props) {
  ...
  useEffect(() => {
    ...
    const onSubmit = async (event) => {
      ...
      if(params.data !== '0') {
        try {
          await axios.post(`${JSON_CONST.DB_URL}master/registrarOffice/update/${sendPost.id}`, sendPost)
            .then((response) => {
              console.log(response);
              setIsSubmitting(false);
              navigate(`/app/master/registrarOffice`, { replace: true });
            })
            .catch((error) => {
              setIsSubmitting(false);
              console.log(error);
          ...
          try {
            await axios.post(`${JSON_CONST.DB_URL}master/registrarOffice/create`, sendPost)
              .then((response) => {
                console.log(response);
                setIsSubmitting(false);
                navigate(`/app/master/registrarOffice`, { replace: true });
